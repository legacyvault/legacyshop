...
import { BreadcrumbItem, IDivisions, IStocks } from '@/types';
...
interface IDialog {
    ...
    //Inertia’s useForm
    data: IStockFormData;
    setData: (field: 'id' | 'division_id' | 'remarks' | 'quantity', value: string) => void;
...
export default function ViewDivision() {
    ...
    const editSubmitHandler = (e: any) => {
        e.preventDefault();
        post(route('division.update-stock'), {
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
                            <div className="text-center">
                                ...
                                <div className="text-4xl font-bold text-primary">{selectedDivision.total_stock}</div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-lg font-semibold text-card-foreground">{selectedDivision.name}</p>
                                </div>

                                {/* Description */}
                                <div className="space-y-2 md:col-span-2">
                                    <h3 className="text-sm font-medium tracking-wide text-muted-foreground uppercase">Description</h3>
                                    <p className="leading-relaxed font-semibold text-card-foreground">{selectedDivision.description}</p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xl font-bold text-primary">{selectedDivision.discount} %</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    ...
                                    <tbody className="divide-y divide-border">
                                        {selectedDivision.stocks.length > 0 ? (
                                            selectedDivision.stocks.map((item, index) => (
...
function AddStockDialog({ open, isOpen, type, onSubmit, data, setData, errors, divisionId, stock }: IDialog) {
    useEffect(() => {
        if (open) {
            if (type === 'edit') {
                setData('id', stock!.id);
